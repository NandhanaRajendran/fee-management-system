...
exports.assignHod = async (req, res) => {
  try {
    ...
    // 🔐 STEP 3: Handle login
    const password = generatePassword();
    const username = `hod_${dept.name.toLowerCase()}`;
...
exports.assignAdvisor = async (req, res) => {
  try {
    ...
    // assign advisor
    if (!dept.advisors) dept.advisors = {};
    dept.advisors[semester] = faculty._id;
...
exports.getStudentDues = async (req, res) => {
    try {
        ...
        const dues = await Due.find({ student: student._id })
...
exports.getMyDues = async (req, res) => {
    const dues = await Due.find({ student: req.user.refId })
        .populate("feeSection", "name");
...
exports.getAdvisorDues = async (req, res) => {
    ...
    const advisor = dept.advisors.find(a =>
        a.faculty.toString() === req.user.refId.toString()
    );
...
exports.getHodDueSheet = async (req, res) => {
    try {
        ...
        dues.forEach(due => {
            ...
            } else if (feeName.includes("fine") || feeName.includes("penalty") || feeName.includes("damage")) {
